    ...
    def extract_from_idl_directory(self,analysis_dir,tsize_targ=None,tsize_cal=None,
        ...
        for ix,f in enumerate(bs_files):
            ...
            if cal_ix[ix]:
                ...
                u_coords1 = uflip*u_coords*np.cos(pa_rad)  + v_coords*np.sin(pa_rad)
                v_coords1 = -uflip*u_coords*np.sin(pa_rad) +  v_coords*np.cos(pa_rad)
    ...
    def make_proj(self,cov_matrix,plot=True,n_remove=2,n_significant=None,tol=1e-2,
        ...
        for wav_ix,wav in enumerate(self.wavel):
            ...
            if n_significant:
                ...
                if not silent:
                    print('  Found '+str(np.sum(good_evals))+' eigenvalues above tolerance')            
...
#            evects[:,ix] /= np.sqrt(np.sum(evects[:,ix]**2)) # Make sure they have length 1 (i.e. normal)
        ...
        proj = np.array(proj)
        self.proj = proj
        self.n_good = np.sum(good_evals) - n_remove # this assumes its the same number for all data...
        ...
        for obs_ix in range(self.nobs):
            ...
            proj_t3err[:,obs_ix,:] = np.nanstd(proj_cps,axis=1) / np.sqrt(proj_cps.shape[1]) # SEM
