...
def cp_loglikelihood_cov(params,u,v,wavel,t3data,cov_inv,model='constant'):
    ...
    cps = cp_model(params,u,v,wavel,model=model)
    resids=t3data-cps
    chi2=resids.transpose().dot(cov_inv).dot(resids)
    loglike = -chi2/2
    return loglike
...
def cp_loglikelihood_proj(params,u,v,wavel,proj_t3data,proj_t3err,proj):
    '''Calculate loglikelihood for projected closure phase data.
    Used both in the MultiNest and MCMC Hammer implementations.'''
    cps = cp_model(params,u,v,wavel)
    n_runs=np.shape(proj_t3data)[1]
    cps = np.reshape(cps,(len(cps)/n_runs,n_runs),order='F') #order checked by ACC
    proj_mod_cps=(np.transpose(proj).dot(cps))[:len(proj_t3data),:] #take only independent clps
...
def cp_loglikelihood_multiple(params,u,v,wavel,t3data,t3err,model='constant',ncomp=1):
    ...
    for ix in range(1,ncomp):
        cps+=cp_model(params[ix*3:(ix+1)*3],u,v,wavel,model=model) # 3 since there are 3 parameters per companion
...
def cp_model(params,u,v,wavels,model='constant'):
    ...
    if model == 'constant':
        ...
        cons=params[2]+ (params[3]-params[2])*(wavels-wavels[0])/(wavels[-1]-wavels[0])
    elif model == 'ndof':
        ndof=params[2:].size
    ...
    ndata = u.size/3 # I changed this to get the disk model working
    #If we have multiple wavelengths just repeat the u and v coords with different wavelengths
    cps=[]
    for ix,con in enumerate(cons):
        phases = phase_binary(u.ravel(),v.ravel(),wavels[ix],[params[0],params[1],con])
    ...
    if plot==True:
        ...
        for i in range(ndim):
            ...
            if i < 3:
                ...
                plt.clf()
                color='k'
            plt.hist(chain[:,i], np.int((niters)**(0.5)),color=color, histtype="step")
            #plt.hist(chain[:,i],color=color, histtype="step",bins=bayesian_blocks.blocks(chain[:,i]))
...
def nest(cpo,paramlimits=[20.,250.,0.,360.,1.0001,10],ndim=3,resume=False,eff=0.3,multi=True,
    ...
    if plot:
        ...
        for i in range(n_params):
            	...
            	p.plot_marginal(i, with_ellipses = True, with_points = False, grid_points=20)
...
def detec_sim_loopfit(everything):
    ...
    for j,th in enumerate(everything['ths']):
        for k,con in enumerate(everything['cons']):
            ...
            chi2_diff = np.sum(np.sum( (resids**2 - rnd_cp**2)/ everything['error'][:,:,np.newaxis]**2,axis=0),axis=0)
...
def detec_sim_loopfit_proj(everything):
    ...
    for j,th in enumerate(everything['ths']):
        for k,con in enumerate(everything['cons']):
            ...
            bin_cp=np.reshape(bin_cp,(len(bin_cp)/n_runs,n_runs),order='F')
            proj_bin_cp=np.transpose(everything['proj']).dot(bin_cp)
            ...
            rnd_cp = proj_bin_cp[:,np.newaxis] + everything['error'][0:n_clps*n_runs,np.newaxis]*everything['rands']
            chi2_sngl = np.sum((((rnd_cp)/ everything['error'][:,np.newaxis])**2),axis=0) 
            chi2_binr = np.sum((((rnd_cp-proj_bin_cp[:,np.newaxis]) / everything['error'][:,np.newaxis])**2),axis=0)
            chi2_diff=chi2_binr-chi2_sngl
            detec_count[j,k]=(chi2_diff <0.0).sum()#this counts the number of detections
    return detec_count
...
def detec_limits(cpo,nsim=2000,nsep=32,nth=20,ncon=32,smin='Default',smax='Default',
    ...
    if projected==True:
        proj=cpo.proj
        cpo.t3err=np.sqrt(cpo.t3err**2+extra_error**2)
        error = cpo.proj_t3err.ravel()*err_scale
        n_clps=cpo.n_good
        n_runs=cpo.n_runs
    elif icpo==True:
        proj=[]
    ...
    if linear_in_mags:
        cons=10**(np.linspace(2.5*np.log10(cmin),2.5*np.log10(cmax),ncon)/2.5)
    ...
    if use_cov==True:
        ...
        rands = np.random.randn(n_clps*n_runs,nwav,nsim)
        error=np.reshape(error,(n_clps*n_runs,nwav))
...
def brute_force_chi2_grid(everything):
    ...
    for j,th in enumerate(everything['ths']):
        for k,con in enumerate(everything['cons']):
            ...
            for i in range(nsim):
                chi2[j,k,i]=np.sum(((sim_cps[:,:,i]-mod_cps) /everything['error'])**2)
    return chi2
...
def brute_force_chi2_grid_proj(everything):
    ...
    for j,th in enumerate(everything['ths']):
        for k,con in enumerate(everything['cons']):
            ...
            proj_bin_cp=proj_bin_cp.ravel()
            for i in range(nsim):
                chi2[j,k,i]=np.sum(((sim_cps[:,:,i]-proj_bin_cp) /everything['error'])**2)
    return chi2
...
def brute_force_detec_limits(cpo,nsim=100,nsep=32,nth=20,ncon=32,smin='Default',smax='Default',
    ...
    lm_fit option allows an extra step involving a levenburg-marquadt fit at each point,
...
def chi2_grid(everything):
    ...
    for j,th in enumerate(everything['ths']):
        for k,con in enumerate(everything['cons']):
            ...
            chi2[j,k]=np.sum(((data_cp.ravel()-mod_cps.ravel())/cpo.t3err.ravel())**2)       
    return chi2
...
def coarse_grid(cpo,nsep=32,nth=20,ncon=32,smin='Default',smax='Default',
    ...
    if threads ==0:
        toc=time.time()
        for ix,sep in enumerate(seps):
            everything={'sep':seps[ix],'cons':cons,'ths':ths,'cpo':cpo,'ix':ix}
            if projected:
    ...
    #sep_pa=np.amin(like,axis=2)
     #sep_crat=np.amin(like,axis=1)
    #pa_crat=np.amin(like,axis=0)
    ...
    plt.figure(0)
    plt.clf()
    plt.imshow(sep_pa,extent=[np.amin(ths),np.amax(ths),np.amin(seps),np.amax(seps)],aspect='auto')
    plt.colorbar()
    ...
    plt.figure(1)
    plt.clf()
    plt.imshow(sep_crat,extent=[np.amin(cons),np.amax(cons),np.amin(seps),np.amax(seps)],aspect='auto')
    plt.colorbar()
    plt.ylabel('Separation (mas)')
    plt.xlabel('Contrast Ratio')
    plt.title('Sep vs Crat')
    plt.draw()
 
    plt.figure(2)
    plt.clf()
    plt.imshow(pa_crat,extent=[np.amin(cons),np.amax(cons),np.amin(ths),np.amax(ths)],aspect='auto')
    plt.colorbar()
    plt.ylabel('Position Angle (deg)')
    plt.xlabel('Contrast Ratio')
    plt.title('PA vs Crat')
    plt.draw()
    plt.show()
...
def multiple_companions_hammer(cpo,ivar=[[50., 0., 2.],[50.,90.,2.]],ndim='Default',
    ...
    if crat_prior != None:
        for c_ix in range(ncomp):
            for ix in range(nparam-2):
                wh=(cs[:,c_ix,ix] >= crat_prior[0]) & (cs[:,c_ix,ix] <= crat_prior[1])
                seps=seps[wh]
                ths=ths[wh]
                cs=cs[wh,:,:]
    ...
    data={'sep':meanseps,'delsep':dseps,'pa':meanths,'delpa':dths,'con':meancs,'delcon':dcs,'chain':sampler.chain,'seps':seps}
    # and clean up
    sampler.pool.terminate()
    return data
    ...
    if logcrat_prior:
        ...
        def myprior(cube, ndim, n_params,paramlimits=paramlimits,kpo=0,n_comp=n_comp):
            ...
            for comp_ix in range(0,n_comp):
                cube[3*comp_ix+0] = cube[3*comp_ix+0]*(paramlimits[comp_ix][1] - paramlimits[comp_ix][0])+paramlimits[comp_ix][0]
                cube[3*comp_ix+1] = cube[3*comp_ix+1]*(paramlimits[comp_ix][3] - paramlimits[comp_ix][2])+paramlimits[comp_ix][2]
                cube[3*comp_ix+2] = cube[3*comp_ix+2]*(paramlimits[comp_ix][5] - paramlimits[comp_ix][4])+paramlimits[comp_ix][4]

         
    def myloglike(cube, ndim, n_params):
        loglike = cp_loglikelihood_multiple(cube,cpo.u,cpo.v,cpo.wavel,cpo.t3data,cpo.t3err,ncomp=n_comp)
        return loglike
